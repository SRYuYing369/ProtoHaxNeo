...
class EntityPlayerSP(private val session: GameSession, override val eventManager: EventManager) : EntityPlayer(0L, 0L, UUID.randomUUID(), ""), Listenable {
	...
	fun setMotion(x: Float, y: Float, z: Float){
	...
	val newDirection: Vector2d
		get() {
			var moveVec2d = Vector2d.from(moveForward , -moveStrafing)
			val calcYaw = (rotationYaw + 90) * (PI / 180)
			val c = cos(calcYaw)
			val s = sin(calcYaw)
			moveVec2d = Vector2d.from(moveVec2d.x * c - moveVec2d.y * s, moveVec2d.x * s + moveVec2d.y * c)
	...
	private val handlePacketInbound = handle<EventPacketInbound> {
		...
		if (packet is StartGamePacket) {
			...
			session.movementMode = packet.authoritativeMovementMode
