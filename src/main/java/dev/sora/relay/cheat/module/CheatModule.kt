...
abstract class CheatModule(val name: String,val chinese: String, val category: CheatCategory,
    ...
    open fun onEnable() {
		session.chat("§l§e${if(session.chineseMode)chinese else name}§r Enable ■")
	}

    open fun onDisable() {
		session.chat("§l§e${if(session.chineseMode)chinese else name}§r Disable □")
	}
