...
class ModuleScaffold : CheatModule("Scaffold","自动搭路", CheatCategory.MOVEMENT) {

    private var swingValue by listValue("Swing", EntityPlayerSP.SwingMode.values(), EntityPlayerSP.SwingMode.BOTH)
	private var extendValue by intValue("Extend",1,0..10)
	private var keepYValue by boolValue("KeepY",true)
	...
	private val handleTick = handle<EventTick> { event ->
		...
		var possibilities = searchBlocks(session.thePlayer.posX, session.thePlayer.posY - EntityPlayer.EYE_HEIGHT,
			session.thePlayer.posZ, 1, session.thePlayer, world, airId)
		if(keepYValue) possibilities = searchBlocks(session.thePlayer.posX, launchY - EntityPlayer.EYE_HEIGHT,session.thePlayer.posZ, 1, session.thePlayer, world, airId)
